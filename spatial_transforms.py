...
#     [0, 255] to a torch.FloatTensor of shape (H x W x C) in the range [0.0, 1.0].
#     """
...
#             # handle numpy array
#             img = ms.Tensor(pic, ms.float32) #.transpose((2, 0, 1))
#             # backward compatibility
#             return img/self.norm_value

#         # if accimage is not None and isinstance(pic, accimage.Image):
#         #     nppic = np.zeros(
#         #         [pic.channels, pic.height, pic.width], dtype=np.float32) # [pic.channels, pic.height, pic.width]
#         #     pic.copyto(nppic)
#         #     return ms.tensor(nppic)

#         # handle PIL Image
#         if pic.mode == 'I':
#             img = ms.Tensor(np.array(pic, np.int32, copy=False))
#         elif pic.mode == 'I;16':
#             img = ms.Tensor(np.array(pic, np.int16, copy=False))
#         else:
#             # img = torch.ByteTensor(torch.ByteStorage.from_buffer(pic.tobytes()))
#             raise RuntimeError('Invalid image mode!!')
